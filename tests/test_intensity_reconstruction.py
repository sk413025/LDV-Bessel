...
def test_define_space_time_grid(reconstructor):
    ...
    assert y.shape == (64,)
    assert t.shape == (10,)
    assert X.shape == (64, 64, 10)
    assert Y.shape == (64, 64, 10)
    assert T.shape == (64, 64, 10)
...
def test_define_measurement_region(reconstructor):
    ...
    assert measurement_mask.shape == (64, 64)
...
def test_generate_interference_intensity(reconstructor):
    ...
    assert intensity.shape == (64, 64, 10)
...
def test_reconstruct(reconstructor):
    ...
    assert s_est.shape == (181,)
    assert isinstance(reconstruction_error, float)
    assert 0 <= reconstructed_theta <= np.pi/2